		...
		try {
			...
			switch (searchProvider || SEARCH_PROVIDER) {
				...
				case "arxiv": {
					...
					results =
						(await search(query, searchProvider, num, meta, context.tokenTracker)).response.results || []
					break
				}
				case "duck":
			...
			if (
				...
				(searchProvider === "jina" || searchProvider === "arxiv")
			) {
...
`)
			...
			if (teamSize > 1) {
				...
				if (subproblems.length > 1) {
					...
					const subproblemResponses = await Promise.all(
					...
					thisStep = {
						...
						imageReferences: subproblemResponses
							.map((r) => (r.result as AnswerAction).imageReferences)
							.filter(Boolean)
							.flat(),
						isFinal: true,
						isAggregated: true,
					} as AnswerAction
					candidateAnswers = subproblemResponses
						.map((r) => (r.result as AnswerAction).mdAnswer)
						.filter((a) => a) as string[]
					// dedup references by their urls
					const uniqueURLs = new Set(thisStep.references.filter((r) => r?.url).map((r) => r.url))
					thisStep.references = Array.from(uniqueURLs).map((url) =>
						(thisStep as AnswerAction).references.find((r) => r?.url === url),
					) as Reference[]

					// aggregate urls
					visitedURLs.push(...subproblemResponses.map((r) => r.readURLs).flat())
					weightedURLs = subproblemResponses
						.map((r) => r.allURLs.map((url) => ({ url, title: "" }) as BoostedSearchSnippet))
						.flat()
...
`)
	...
	if (!(thisStep as AnswerAction).isFinal) {
		logInfo(
			`Beast mode!!! budget ${((context.tokenTracker.getTotalUsage().totalTokens / tokenBudget) * 100).toFixed(2)}%`,
			{
	...
	if (trivialQuestion) {
		...
		answerStep.answer = repairMarkdownFinal(
			convertHtmlTablesToMd(
				fixBadURLMdLinks(
					fixCodeBlockIndentation(
						repairMarkdownFootnotesOuter(
							await finalizeAnswer(answerStep.answer, allKnowledge, context, SchemaGen),
						),
					),
					allURLs,
				),
		...
		if (imageObjects.length && withImages) {
			try {
				answerStep.imageReferences = await buildImageReferences(
					answerStep.answer,
					imageObjects,
					context,
					SchemaGen,
		...
		if (withImages && answerStep.imageReferences?.length) {
			const sortedImages = answerStep.imageReferences.sort(
	...
	// max return 300 urls
	const returnedURLs = weightedURLs
		.slice(0, numReturnedURLs)
		.filter((r) => r?.url)
		.map((r) => r.url)
	return {
		result: thisStep,
...
async function storeContext(
	...
	if (isDevelopment) {
		try {
			...
			const debugDir = process.env.DEBUG_DIR || path.join(os.tmpdir(), "deep-research-debug")
			
			// Ensure directory exists
			await fs.mkdir(debugDir, { recursive: true })
			
			await fs.writeFile(
				path.join(debugDir, `prompt-${step}.txt`),
				`
Prompt:
${prompt}
...
`,
			...
			await fs.writeFile(path.join(debugDir, "context.json"), JSON.stringify(allContext, null, 2))
			await fs.writeFile(path.join(debugDir, "queries.json"), JSON.stringify(allKeywords, null, 2))
			await fs.writeFile(path.join(debugDir, "questions.json"), JSON.stringify(allQuestions, null, 2))
			await fs.writeFile(path.join(debugDir, "knowledge.json"), JSON.stringify(allKnowledge, null, 2))
			await fs.writeFile(path.join(debugDir, "urls.json"), JSON.stringify(weightedURLs, null, 2))
			await fs.writeFile(path.join(debugDir, "messages.json"), JSON.stringify(msgWithKnowledge, null, 2))
			
			logInfo(`Debug files written to: ${debugDir}`)
		} catch (error) {
