...
async function emitRemainingContent(res: Response, requestId: string, created: number, model: string, content: string) {
	if (!content) return
...
const validationRules = [
	...
	body("messages").isArray({ min: 1 }).withMessage('The "messages" parameter is required.'),
]
...
app.post("/v1/chat/completions", validationRules, (async (req: Request, res: Response) => {
	...
	try {
		...
		const annotations = (finalStep as AnswerAction).references
			?.filter((ref) => ref?.url && ref?.title && ref?.exactQuote && ref?.dateTime)
			.map((ref) => ({
